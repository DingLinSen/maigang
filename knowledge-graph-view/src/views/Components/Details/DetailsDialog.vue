...
const toggleFull = (isFull: boolean) => {
  ...
  if (isFull) {
    divHeight.value = layoutContentH - getSearchWrapperHeight() - 70
  } else {
...
<template>
  ...
  >
    <div
      style="width: 100%; height: 100%; font-size: 13px; font-family: Microsoft YaHei"
    ...
    >
      <div :style="{ minHeight: divHeight - 20 * props.url.split(',').length - 30 + 'px' }">
        ...
        <div
          style="padding: 0px 15px; overflow-y: auto; word-break: break-word; min-height: 300px"
          v-if="props.content"
        >
