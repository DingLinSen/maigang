...
const resetTableHeight = () => {
  ...
  if (ElDialogFef.value.isFullscreen) {
    ...
    defaultHeight.value = `${layoutContentH - 35 - topH - footerH}px`
  } else {
    defaultHeight.value = layoutContentH * 0.8 - 35 - topH - footerH + 'px'
  }
}
...
const toggleFull = (isFull: boolean) => {
  if (isFull) {
    ...
    defaultHeight.value = `${documentClientHeight.value - 35 - topH - footerH}px`
  } else {
    defaultHeight.value = documentClientHeight.value * 0.7 + 'px'
  }
}
...
<template>
  ...
  <Dialog
    ref="ElDialogFef"
    :class="props.isWhite ? 'white-home-class' : ''"
    v-model="DialogFlag"
    :width="'55%'"
    append-to-body
