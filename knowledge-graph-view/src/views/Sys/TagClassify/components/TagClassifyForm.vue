...
import { tagTypeAddApi, editKonwledgApi, getFindByIdApi } from '@/api/classification/label'
...
const getDetailsData = async (id?: string | undefined) => {
  ...
  if (res && res.data) {
    ...
    tagTypeData.tagTypeName = res.data.tagTypeName
...
const save = async () => {
  ...
  await elFormRef?.validate(async (isValid) => {
    if (isValid) {
      ...
      data.tagTypeName = data.tagTypeName ? data.tagTypeName : ''
