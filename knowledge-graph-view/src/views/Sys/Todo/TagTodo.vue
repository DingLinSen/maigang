...
import TagTodoForm from './components/TagTodoForm.vue'
...
const AddAction = (row) => {
  TagTodoFormRef.value.openDrawer('', row.id)
}

//搜索
const setSearchParamsSub = (param: Recordable) => {
  params.status = param.status || param.status === 0 ? param.status : null
  params.tagTypeName = param.tagTypeName ? param.tagTypeName : null
...
const resetTableHeight = () => {
  ...
  nextTick(() => {
    ...
    console.log(defaultHeight.value, 999)
  })
}
  ...
  <section>
    ...
    >
      ...
      <template #action="{ row }">
        ...
        <ElButton
          v-if="isAuth('system:labelClass:pass') && row.status == 0"
          class="btn-default"
          :text="true"
          size="small"
          @click="sureClick(row)"
        ...
        <ElButton
          v-if="isAuth('system:labelClass:reject') && row.status == 0"
          class="btn-default"
          :text="true"
          size="small"
    ...
    <TagTodoForm ref="TagTodoFormRef" />
