...
import { tagTypeListApi, delKonwledgApi } from '@/api/classification/knowledge'
...
const AddAction = (row, type?) => {
  ...
  if (type == 'add') {
    if (row) {
      KnowledgeFormRef.value.openDrawer('add', row.id, 1)
    } else {
      KnowledgeFormRef.value.openDrawer('add', '', 0)
    }
  } else {
    if (row.parentId) {
      KnowledgeFormRef.value.openDrawer('edit', row.id, 1)
    } else {
      KnowledgeFormRef.value.openDrawer('edit', row.id, 0)
    }
  }
}
