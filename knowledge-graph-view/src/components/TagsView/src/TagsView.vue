...
const menuData = ref([])
// const menuPath = ref(currentRoute.value.path)
...
watch(
  () => currentRoute.value,
  () => {
    // menuPath.value = currentRoute.value.path
    menuData.value = currentRoute.value.meta.title
  }
)
</script>
...
<template>
  <div class="tags-view-cont">
    ...
    >
      <div class="tag-item">
        <span class="tag-item-text">平台管理 > </span>
        <span class="tag-item-aciive">{{ menuData }}</span>
      </div>
    </div>
  </div>
</template>
