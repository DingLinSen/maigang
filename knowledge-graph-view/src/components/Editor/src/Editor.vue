...
const filrUrl = '/mgdaas-file/upload'
...
editorConfig.MENU_CONF!.uploadImage = {
  async customUpload(file: File, insertFn: InsertFnType) {
    ...
    if (file.name.length > 60) {
      return ElMessage.error('文件名(含后缀)长度不能超过60')
    }
    ...
    let path = PATH_URL + '/mgdaas-file/upload'
    if (props.isCompanyNews) {
      path = PATH_URL + '/oa-market/companyContent/uploadImage'
    }
    axios
      .post(path, formData, {
        headers: {
      ...
      .then((res) => {
        if (props.isCompanyNews) {
          let path: string = res.data.data[0]
          insertFn(path, getFileName(path), path)
        } else {
