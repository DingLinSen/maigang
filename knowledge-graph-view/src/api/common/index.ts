...
export const getDictApi = (): Promise<IResponse> => {
  return request.get({ url: '/knowledge-mapping/management/dict/type/list' })
}
// 获取某个字典
export const getDictOneApi = async (type): Promise<IResponse> => {
  return request.get({ url: '/knowledge-mapping/management/dict/data/type/' + type })
}
// 获取菜单
export const getMenuList = (): Promise<IResponse> => {
  return request.get({ url: '/knowledge-mapping/management/menu/getRouters' })
}
// 公用下载文件
export const docPreview = (): Promise<IResponse> => {
  return request.getFile({ url: '/knowledge-mapping/management/menu/getRouters' })
}
