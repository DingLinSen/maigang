...
export const setTextPlaceholder = (schema: FormSchema): PlaceholderMoel => {
  ...
  if (textMap.includes(schema?.component as string)) {
    return {
      placeholder:
        schema?.componentProps?.disabled || schema?.isDisabled
          ? ''
          : t('common.inputText') + schema.label
    }
  }
  if (selectMap.includes(schema?.component as string)) {
    ...
    if (
      ...
      const start = schema?.componentProps?.type === 'daterange' ? '开始日期' : '开始月份'
      const end = schema?.componentProps?.type === 'daterange' ? '结束日期' : '结束月份'
      return {
        startPlaceholder:
          schema?.componentProps?.disabled || schema?.isDisabled
            ? ''
            : schema?.componentProps?.type !== 'datetimerange'
            ? start
      ...
      return {
        placeholder:
          schema?.componentProps?.disabled || schema?.isDisabled
            ? ' '
            : t('common.selectText') + schema.label
      }
    }
  }
  return {}
 ...
 */
export const initModel = (schema: FormSchema[], formModel: Recordable, prop?: any) => {
