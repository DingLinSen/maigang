...
const { required, validatePhone, notAllSpace } = useValidator()
...
const save = async () => {
  ...
  farmBirthPeriod.value.forEach((item, index) => {
    res.push(farmBirthPeriod.value[index].el.validate())
    farmBirthPeriod.value[index].farmBirthControl.forEach((item0, index0) => {
      res.push(item0.el.validate())
    })
  })
...
const getDetailsData = async (id: string) => {
  ...
  if (res && res.data) {
    ...
    plantData = res.data
    let plantingStandards = planting_standards.value.some(
      (item: any) => item.value == plantData.plantingStandards
    )
    if (!plantingStandards) {
      plantData.plantingStandards = ''
    }
...
// 删除操作
const delCycleOperate = (index, i) => {
  farmBirthPeriod.value[index].farmBirthControl.splice(i, 1)
}
// 删除生育期
...
const rules = reactive({
  days: [
    ...
    { min: 0, max: 15, message: '长度范围1-15', trigger: 'blur' },
    {
  ...
  birthPeriod: [
    { required: true, message: '该项为必填项!', trigger: 'blur' },
    { min: 0, max: 20, message: '长度范围1-20', trigger: 'blur' },
    {
      validator: (_: any, value: string, callback: Callback) => {
        notAllSpace(value, callback, '不能输入纯空格')
      },
  ...
  method: [
    { required: true, message: '该项为必填项!', trigger: 'blur' },
    { min: 0, max: 50, message: '长度范围1-50', trigger: 'blur' },
    {
      validator: (_: any, value: string, callback: Callback) => {
        notAllSpace(value, callback, '不能输入纯空格')
      },
  ...
  standard: [
    { required: true, message: '该项为必填项!', trigger: 'blur' },
    { min: 0, max: 200, message: '长度范围1-200', trigger: 'blur' },
    {
      validator: (_: any, value: string, callback: Callback) => {
        notAllSpace(value, callback, '不能输入纯空格')
      },
...
const refChange = (ref: any, index: number) => {
  if (farmBirthPeriod.value.length > index) {
    farmBirthPeriod.value[index].el = ref
  }
}

const refBirthChange = (ref: any, index: number, index0: number) => {
  if (
    farmBirthPeriod.value[index] &&
    farmBirthPeriod.value[index].farmBirthControl &&
    farmBirthPeriod.value[index].farmBirthControl.length > index0
  ) {
    farmBirthPeriod.value[index].farmBirthControl[index0].el = ref
  }
}
</script>
  ...
  <Dialog v-model="isShow" width="70%" :title="pageTite" @close="closeDrawer">
    <Form :schema="schema" ref="formRef" class="form-content">
      <template #cycle>
        <div v-for="(item, index) in farmBirthPeriod" class="standardForm" :key="index">
          <ElForm :model="item" :rules="rules" :ref="(el) => refChange(el, index)">
            <el-row :gutter="30">
              <el-col :span="12">
                <el-form-item label="天数" prop="days">
                  <el-input :disabled="!isSave" v-model="item.days" />
                </el-form-item>
              </el-col>
              <el-col :span="12"
                ><el-form-item label="生育期" prop="birthPeriod">
                  <el-input :disabled="!isSave" v-model="item.birthPeriod" /> </el-form-item
              ></el-col>
              <el-col :span="24">
                <div v-for="(ele, i) in item.farmBirthControl" :key="i" class="operate_items">
                  <ElForm :model="ele" :rules="rules" :ref="(el0) => refBirthChange(el0, index, i)">
                    <el-row class="items-center">
                      <el-col :span="farmBirthPeriod[index].farmBirthControl.length > 1 ? 10 : 12">
                        <el-form-item
                          :label="i == 0 ? '农事操作' : ''"
                          prop="method"
                          :class="i == 0 ? 'mr-[15px]' : 'mr-[15px] mt-[-10px]'"
                        >
                          <el-input :disabled="!isSave" v-model="ele.method" />
                        </el-form-item>
                      </el-col>
                      <el-col :span="farmBirthPeriod[index].farmBirthControl.length > 1 ? 10 : 12">
                        <el-form-item
                          :label="i == 0 ? '操作规范' : ''"
                          prop="standard"
                          :class="i == 0 ? 'ml-[15px]' : 'ml-[15px] mt-[-10px]'"
                        >
                          <el-input :disabled="!isSave" v-model="ele.standard" />
                        </el-form-item>
                      </el-col>
                      <el-col
                        :span="4"
                        v-if="farmBirthPeriod[index].farmBirthControl.length > 1 && isSave"
                      >
                        <el-form-item
                          :class="i == 0 ? 'ml-[15px] mt-[30px]' : 'ml-[15px] mt-[-10px]'"
                        >
                          <ElButton type="primary" @click="delCycleOperate(index, i)">
                            删除操作
                          </ElButton>
                        </el-form-item>
                      </el-col>
              ...
              <el-col :span="24">
                <ElButton v-if="isSave" type="primary" @click="addCycleOperate(index)">
                  新增操作
                </ElButton>
                <ElButton
                  v-if="farmBirthPeriod.length > 1 && isSave"
                  type="primary"
    ...
    <template #footer>
      <div class="details-footer">
        ...
        <ElButton v-if="isSave" type="primary" @click="addCycle"> 新增生育期 </ElButton>
