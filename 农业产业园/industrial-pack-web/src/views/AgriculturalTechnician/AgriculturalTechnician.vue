...
const searchParamInit = (param) => {
  ...
  params.userType = param.userType || undefined
...
const crudSchemas = reactive<CrudSchema[]>([
  ...
  },
  {
    ...
    formatter: (row: Recordable) => {
      return h('span', row.userType == '01' ? '农户' : row.userType == '02' ? '农技员' : '--')
    }
  },
  {
    field: 'userName',
...
<template>
  ...
  <ContentWrap>
    ...
    >
      <template #operateBtnLeft>
        ...
        <ElButton type="primary" @click="resetPwdModal()" plain>
          <Icon icon="ep:circle-check" class="mr-5px" />
          重置密码
        </ElButton>
      </template>
      <template #action="{ row }">
        <ElButton v-show="!row.admin" type="text" :text="true" @click="action(row, 'edit')">
          编辑
        </ElButton>
        <ElButton v-show="!row.admin" type="text" :text="true" @click="delData(row, false)">
          删除
        </ElButton>
      </template>
    </Table>
  ...
  <!-- 调整部门  -->
  <Dialog v-model="changeDeptVisible" modal-class="dept-tree" :fullscreen="false" title="调整部门">
    <div class="">
      <Tree
        :title="''"
        :isShowExpend="false"
  ...
  <UserForm title="用户" ref="userFormRef" @get-app-user-list="getList()" />
  <!-- 分配角色 -->
