...
const layouts = import.meta.glob('../layout/**/*.{vue,tsx}')
...
export const generateRoutesFn2 = (routes: AppCustomRouteRecordRaw[]): AppRouteRecordRaw[] => {
  ...
  for (const route of routes) {
    if (route.meta) {
      route.meta.noCache = !route.meta?.isCache || route.meta?.isCache === '1' ? true : false
    }
    ...
    if (route.component) {
      const com =
        route.component === 'Layout' || route.component === 'ParentView'
          ? layouts[`../layout/${route.component}.vue`] || layouts[`../layout/Layout.vue`]
  ...
  for (const route of asyncRouterMap) {
    ...
    if (data.meta && data.meta.link) {
      if (data.path.includes('bpmn')) {
        data.path = encodeURIComponent(data.path + '?cookies=' + wsCache.get('Admin-Token'))
      } else {
