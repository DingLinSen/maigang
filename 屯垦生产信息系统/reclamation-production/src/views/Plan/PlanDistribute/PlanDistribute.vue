...
import { PlanDistributeForm } from './components/index'
import { DistributeForm } from './components/index'
...
const PlanDistributeFormRef = ref<ComponentRef<typeof PlanDistributeForm>>()
const DistributeFormRef = ref<ComponentRef<typeof DistributeForm>>()
// useEmitt({
...
const action = (row: any, type: string) => {
  nextTick(() => {
    PlanDistributeFormRef.value?.openDrawer(type, row.id)
  })
}
const distribute = () => {
  nextTick(() => {
    DistributeFormRef.value?.openDrawer()
  })
}
  ...
  <template v-if="!isAdd">
    ...
    <ContentWrap>
      ...
      >
        ...
        <template #action="{ row }">
          <ElButton type="text" @click="distribute" :text="true" v-hasPermi="'system:post:edit'">
            分发
          </ElButton>
          <ElButton
            type="text"
    ...
    <PlanDistributeForm
      title="规(计)划分发"
      ref="PlanDistributeFormRef"
      @get-class-room-list="getList()"
    />
    <DistributeForm title="规(计)划分发" ref="DistributeFormRef" @get-class-room-list="getList()" />
  </template>
  <template v-else>
    <div class="custom">
      <div class="btn">
