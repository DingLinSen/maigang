...
import { ExternalDataForm, seeTable } from './components/index'
...
const ExternalDataFormRef = ref<ComponentRef<typeof ExternalDataForm>>()
const seeTableRef = ref<ComponentRef<typeof seeTable>>() // 查看表格
// useEmitt({
...
const addAction = () => {
  nextTick(() => {
    ExternalDataFormRef.value?.openDrawer('add', '')
  })
}
...
const action = (row: any, type: string) => {
  if (type === 'see') {
    nextTick(() => {
      seeTableRef.value?.openDrawer(row.id)
    })
  } else {
    nextTick(() => {
      ExternalDataFormRef.value?.openDrawer(type, row.id)
    })
  }
}
...
<template>
  ...
  <ExternalDataForm title="外部数据" ref="ExternalDataFormRef" @get-class-room-list="getList()" />
  <seeTable ref="seeTableRef" @get-class-room-list="getList()" />
</template>
