...
const submit = () => {
  if (!props.isTrans) {
    ...
    if (userData.value.length) {
      let deptId = getUserDept(userData.value[0])
      let isSame = userData.value.every((v) => {
        let deptIds = getUserDept(v)
        return deptId == deptIds
      })
      if (isSame) {
        returnSelect()
      } else {
        ElMessage.warning('请选择同一部门人员')
      }
    } else {
      returnSelect()
    }
