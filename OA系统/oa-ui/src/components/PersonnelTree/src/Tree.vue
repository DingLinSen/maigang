    ...
    <template #default="{ node, data }">
      <span class="custom-tree-node">
        ...
        <img
          v-if="!data.children"
          @click="opend(node)"
          src="@/assets/imgs/keepAttend/ej_icon.png"
          alt=""
        />
...
.departmentTree::-webkit-scrollbar {
  width: 6px; /*  设置纵轴（y轴）轴滚动条 */
  height: 6px; /*  设置横轴（x轴）轴滚动条 */
}
/* 滚动条滑块（里面小方块） */
.departmentTree::-webkit-scrollbar-track {
...
/*定义滑块 内阴影+圆角*/
.departmentTree::-webkit-scrollbar-thumb {
  border-radius: 3px;
  opacity: 0.3;
  -webkit-box-shadow: inset 0 0 20px rgba(0, 0, 0, 0.1);
  background-color: rgba(168, 171, 178, 0.4);
}
</style>
