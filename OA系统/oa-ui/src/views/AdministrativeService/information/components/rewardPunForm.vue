...
import { ref, unref, reactive, getCurrentInstance, watch } from 'vue'
...
import {
  addCompanyInformationApi,
  editCompanyInformationApi,
  getDetailCompanyInformationApi
...
watch(
  ...
  (val) => {
    if (checked.value) {
      setValues({ user: 0 })
    } else if (userData.value.length >= 1) {
      setValues({ user: 0 })
    } else {
      setValues({ user: '' })
    }
  ...
  await elFormRef?.validate(async (isValid) => {
    if (isValid) {
      ...
      if (checked.value) {
        ...
        data.allStatus = 1
        data.companyUserDos = userData.value.map((v) => {
          return { userId: v }
        })
      }
...
const editDict = async (data: any) => {
  const res = await editCompanyInformationApi(data)
    .catch(() => {})
    .finally(() => {
      setTimeout(() => {
        loading.value = false
...
const addDict = async (data: any) => {
  const res = await addCompanyInformationApi(data)
    .catch(() => {})
    .finally(() => {
      setTimeout(() => {
        loading.value = false
...
//获取详情
const getDetailsData = async (row: any) => {
  detailsId.value = row.id
  checked.value = row.allStatus == 0 ? true : false
  if (checked.value) {
    setValues({ user: 0 })
  }
  const res = await getDetailCompanyInformationApi(row.id)
  if (res && res.data) {
    rewardPunData = res.data
    if (rewardPunData.companyUserDos) {
      userData.value = rewardPunData.companyUserDos.map((v) => v.userId)
    }
...
<template>
  ...
  >
    <Form label-position="top" :schema="schema" ref="formRef" :disabled="!isSave">
      <template #user>
        <div class="checkbox-all">
          <el-checkbox @change="handleChangePeo" v-model="checked" label="全体人员" />
        </div>
        <div class="div-person-bg" v-if="!checked">
    ...
    <template #footer>
      <div class="footer">
        <el-button type="primary" v-if="isSave" :loading="loading" @click="save">提交</el-button>
