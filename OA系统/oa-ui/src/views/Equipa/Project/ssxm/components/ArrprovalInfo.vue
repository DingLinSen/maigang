...
import { getEstablishApi, auditProjectApi, changeAuditApi } from '@/api/project/ssxm'
...
const getDetailsData = async (id: string) => {
  ...
  if (res && res.data) {
    ...
    implementData.value.projectType = detailsData.value.projectType
    ...
    auditLisData.forEach((item: any) => {
      ...
      if (item.flag === '1') {
        ...
        c = '发起了' +( detailsData.value.updateProjectTitle ? detailsData.value.updateProjectTitle :detailsData.value.projectTitle) + b + '申请'
      } else if (item.flag === '2') {
        // 审核
        if (!item.status && item.status !== '') {
          ...
          c =
            ...
            ( detailsData.value.updateProjectTitle ? detailsData.value.updateProjectTitle :detailsData.value.projectTitle) +
            b +
            '申请' +
            '，建议/理由：' +
    ...
    if (res.data.status === '1') {
      ...
      if (res.data.auditSort === '0' || !res.data.auditSort) {
        ...
        stepsList.value.push({
          status: '', // wait、process、finish、error，
          title: data.projectManagerName ? data.projectManagerName : '',
          content: '结束转实施申请，待补充。',
          icon: ''
        })
        stepsList.value.push({
          status: '', // wait、process、finish、error，
          title: data.salesManagerName ? data.salesManagerName : '',
          content: '结束转实施申请，待补充。',
          icon: ''
        })
        stepsList.value.push({
      ...
      // 转实施申请 未审批时 ----销售经理补充
        ...
        if (data.projectManagerLeaderName !== data.salesManagerLeaderName) {
          stepsList.value.push({
            status: '', // wait、process、finish、error，
            title: data.salesManagerLeaderName,
            content: '结束转实施申请，待审批。',
        ...
        if ('张保国' !== data.salesManagerLeaderName) {
          stepsList.value.push({
            status: '', // wait、process、finish、error，
            title: '张保国',
            content: '结束转实施申请，待审批。',
      ...
      // // 变更申请 未审批时 ----上级负责人已确认
      else if (data.changeSort === '3' || data.changeSort === '4') {
        stepsList.value.push({
          status: '', // wait、process、finish、error，
          title: '张保国',
          content: '项目变更申请，待审批。',
...
// 立项申请表
const toImplement = () => {
  approvalDialog.value.isFullscreen = false
    ...
    <template #content>
      ...
      <div v-if="isFlag !== 'process'">
        <Segmentation title="申请信息">
          <template #content>
            <div class="draw-wrap">
              <p class="apply-info-p" v-if="detailsData.status == '1'"
                >申请
                ...
                项目立项转实施。</p
              >
              <p class="apply-info-p" v-if="detailsData.status == '5'"
                >申请
                <span style="font-weight: 600">{{
                  nameOverflow(detailsData.projectTitle, 30)
                }}</span>
              ...
              <div class="apply-wrap">
                <p class="apply-content-p">
                  内容：申请 {{ detailsData.projectTitle }}
                  {{
                    detailsData.status == '1'
                      ? '项目立项转实施' +
                        (detailsData.updateProjectTitle
