...
import { getAfterSalesApi, endProjectAuditApi } from '@/api/project/afterSales'
...
const openDrawer = (flag: string, id: string) => {
  ...
  formData.value.reason = null
  isFlag.value = flag
  pageTite.value = flag === 'endAudit' ? '审批' : '审批进程'
  ...
  <DrawerWrap :title="pageTite" :isShow="isShow" ref="drawerWrap">
    <template #content>
      ...
      <Segmentation
        ...
        :isShowtitle="isFlag !== 'end'"
      >
        <template #content>
          <div class="form-wrap" v-if="isFlag !== 'end'">
            <ElForm
              :model="formData"
              :rules="ruleValidate"
              ref="formRef"
          ...
          <div v-if="isFlag !== 'end'" class="line-class"></div>
          <!-- 步骤条 -->
