  ...
  <section class="pr-2 bg-white addform mt-[36px] pb-[36px] border-b-1">
    ...
    <div class="addform-content">
          ...
          <el-row>
            ...
            <el-col :span="2" class="flex mt-[30px] justify-center">
              <el-button
                ...
                @click="del(index)"
                v-if="
                  (props.title === '工作经历（公司内部）' || props.title === '健康状况') &&
                  !forbidden
                "
      ...
      <div v-else class="new-add-form bg-[#f5fbfe] h-[50px]">
        <div
          class="cursor-pointer justify-center flex items-center h-full"
          v-if="
            (props.title === '工作经历（公司内部）' || props.title === '健康状况') && !forbidden
          "
          @click="add()"
        >
          <img class="mr-5px" src="@/assets/imgs/icon/add_bar_icon1.png" />
          <span class="text-[#00a0e9]">请添加{{ props.title }}</span>
        </div>
