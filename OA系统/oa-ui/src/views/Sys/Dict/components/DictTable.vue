...
//打开drawer并初始化
const tableRef = ref()
const divHeight = ref<number>(document.documentElement.offsetHeight * 0.7)
...
const getSearchWrapHeight = (): number => {
  let topH = 0
  ...
  if (buttonWrapper && buttonWrapper.length) {
    buttonWrapper.forEach((ele: any) => {
      topH = topH > 0 ? topH : topH + ele.clientHeight
    })
  }
  const dicSearch = document.querySelector('.dic-search')
  if (dicSearch) {
    topH = topH + dicSearch.clientHeight
  }
  return topH
}

const getDrawerHeaderHeight = (): number => {
  let topH = 0
  const topWrap = document.querySelector('.el-drawer__header')
  if (topWrap) {
