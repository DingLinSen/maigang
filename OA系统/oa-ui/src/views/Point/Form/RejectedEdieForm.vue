...
import { editPointManageApi, getPointManageDetailApi } from '@/api/point'
...
const save = async () => {
  ...
  await formRef.value?.validate(async (isValid: boolean) => {
    if (isValid) {
      loading.value = true
      const res = await editPointManageApi(ruleForm.value)
        .catch(() => {})
        .finally(() => {
          setTimeout(() => {
            loading.value = false
...
<template>
  ...
  >
    ...
    >
      <el-row>
        <el-col :span="24">
          <el-form-item label="详细事由" prop="supplement">
            <el-input
              type="textarea"
