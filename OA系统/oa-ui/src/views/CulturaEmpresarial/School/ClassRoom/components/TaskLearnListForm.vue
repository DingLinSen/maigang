...
const getSearchWrapperHeight = (): number => {
  //搜索框加操作栏高度
  let topH = 0
  const searchWrapper = document.querySelectorAll('.searchWrapperTask')
...
const getSearchWrapperWidth = (): number => {
  let topH = 0
  const searchWrapper = document.querySelector('.searchWrapperTask')
  if (searchWrapper) {
...
const toVideoDetails = (item: any) => {
  // if (item.delay == 0) {
  //   courseDetailsRef.value.openDrawer(item.id, true, item.assessId)
  // } else {
  //   courseDetailsRef.value.openDrawer(item.id, false, item.assessId)
  // }
  if (item.delay == 0) {
...
<template>
  <DrawerWrap ref="drawerWrap" :isShow="isShow" title="任务学习">
    <template #content>
      <div v-if="isAuth('school:task:query')" ref="searchWrapper" class="searchWrapperTask">
        <el-input
        ...
        <el-select
          v-model="searchParams.delay"
          class="input-class ml-[12px]"
          clearable
          placeholder="请选择延误状态"
        >
          <el-option label="正常" value="0" />
          <el-option label="延误" value="1" />
        </el-select>
              ...
              <div class="col-box" @click="toVideoDetails(item)">
                ...
                <div class="detail-wrap">
                  ...
                  <div class="deadline-class time-box">
                    <span class="time">截止时间：{{ item.endDate ? item.endDate : '--' }}</span>
                    <span
                      v-if="(item.userViewStatus == 0 || !item.userViewStatus) && item.delay == 0"
