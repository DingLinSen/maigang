<!--
 * @description planyear- 登记统计-周训练档案--index
 * @author yxm
 * @date 2022-04-07
 * -->
<template lang="html">
...
import {initRecordWeekList,deleteRecordWeekByIds} from '@/api/statistics/week/api-modules-week'
...
export default {
  ...
  data() {
    return {
      canSearch: this.isAuth('statistics:week:query'), // 查询
      canReset: this.isAuth('statistics:week:query'), // 重置
      canSave: this.isAuth('statistics:week:add'), // 添加
      canEdit: this.isAuth('statistics:week:edit'), // 编辑
      canExport: this.isAuth('statistics:week:export'),
      canDelete: this.isAuth('statistics:week:delete'), // 删除
  ...
  methods: {
    ...
    initListData() {
      ...
      this.loading = true
      initRecordWeekList(this.searchParams)
        .then(res => {
          // console.log(res)
          if (res.data.status) {
