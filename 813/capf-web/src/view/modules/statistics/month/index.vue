<!--
 * @description planyear- 登记统计-月训练档案--index
 * @author yxm
 * @date 2022-04-07
 * -->
<template lang="html">
...
import {initRecordMonthList} from '@/api/statistics/month/api-modules-month'
...
export default {
  ...
  data() {
    return {
      ...
      canSave: false, // 添加
      canEdit: false, // 编辑
      canExport: this.isAuth('statistics:month:export'),
  ...
  methods: {
    // 初始化数据表格
    initListData() {
      if (this.searchParams.taskTime) {
        this.searchParams.taskTime =
          this.$moment(this.searchParams.taskTime).format('YYYY-MM')
      }
      this.loading = true
      initRecordMonthList(this.searchParams)
        .then(res => {
          // console.log(res)
          if (res.data.status) {
    ...
    loadExportParams() {
      ...
      let params = {
        taskTime: this.searchParams.taskTime
          ? this.searchParams.taskTime
          : null
      }
      return params
