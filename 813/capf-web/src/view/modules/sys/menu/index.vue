...
<script>
import { initMenuList, deleteMenuByIds } from '@/api/sys/api-modules-menu'
...
export default {
  ...
  data() {
    return {
      canReset: false, // 重置
      canSearch: false, // 查询
      canSave: this.isAuth('sys:menu:add'), // 添加
      canEdit: this.isAuth('sys:menu:edit'), // 编辑
      canDelete: this.isAuth('sys:menu:delete'), // 删除
  ...
  methods: {
    ...
    initListData() {
      ...
      initMenuList()
        .then(res => {
          this.spinShow = false
          if (res.data.status) {
            var dataItems = res.data.data
            var tempList = treeDataTranslate(sortByKey(dataItems, 'orderNum'), 'menuId')
    ...
    async handleDelete() {
      ...
      deleteMenuByIds({ ids: ids.substring(0, ids.length - 1) }).then(res => {
        if (res.data.status) {
    ...
    rowAddClick() {
      if (this.chooseListData && this.chooseListData.length > 0) {
        var result = this.chooseListData[0]
        if (result.deptType === '2' || result.deptType === '4') {
          this.$Message.error('暂不可创建子级')
        } else {
          if (!result._level || result._level < 3) {
