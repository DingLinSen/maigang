  ...
  <div class="details-container">
    <div class="control-container">
      <div class="control-top">
        <div class="one">
          <span>考场名称：</span>{{ details.examinationRoomName || '--' }}
          &nbsp;&nbsp;<span>考试单位：</span>{{ details.examinationDeptName || '--' }} &nbsp;&nbsp;
        </div>
        <div class="two">
      ...
      <div class="control-content" :style="{ maxHeight: controlHight - 120 + 'px' }">
        <div class="card-wrap" v-for="(item, index) in listData.terminalList" :key="index"
          :style="{ background: item.ipAddress ? '#ffffff' : '#F4F4F4' }">
          <div class="card-top" style="background: #F9F9F9">
            <div class="left">
              <div>
                <img class="dec" src="@/assets/images/sb_icon.png" alt="" v-if="item.ipAddress">
                <img class="dec" src="@/assets/images/sb_offline_icon.png" alt="" v-else>
                <span class="dec-name" :style="{ color: item.userId ? '#333333' : '#ABABAB' }">{{ item.userName }}</span>
                <!-- 0 未认证 1认证 默认0 -->
            ...
            <div class="right">
              <span class="status on" v-if="item.ipAddress"><img src="@/assets/images/common/online_icon.png"
                  alt="">已上线</span>
              <span class="status dowm" v-else><img src="@/assets/images/common/downline_icon.png" alt="">已下线</span>
              <Button type="error" size="small" @click="forcedOffline(item)" v-if="item.ipAddress">强制下线</Button>
            </div>
            <div class="right" v-if="item.ipAddress">
              <span class="icon iconfont icon-photo_selected_icon" style="font-size: 22px"
                @click="takePhoto(item)"></span>
            </div>
          </div>
          <div class="card-bottom" :style="{ background: item.ipAddress ? '#ffffff' : '#F4F4F4' }">
            <div class="card-left">
              <p>
                <!--                <span class="name" :style="{color: item.userId ? '#222222' : '#ACACAC'}">{{item.userName || '&#45;&#45;'}}</span>-->
                <span class="sub">ip：</span>
                <span class="dept" style="margin-right: 10px"
                  :style="{ color: item.ipAddress ? '#222222' : '#ACACAC' }">{{
                    item.ipAddress || '--' }}</span>
                <span class="sub">所属：</span>
                <span class="dept" :style="{ color: item.ipAddress ? '#222222' : '#ACACAC' }">{{ item.deptName || '--'
                }}</span>
              </p>
              <p>
                <span class="sub">当前考试：</span>
                <span class="dept" style="margin-right: 10px"
                  :style="{ color: item.ipAddress ? '#222222' : '#ACACAC' }">{{
                    examinationName }}</span>
                <!--                <span class="sub">进度：</span>-->
                <!--                <span class="dept" :style="{color: item.ipAddress ? '#222222' : '#ACACAC'}"><span style="color:#26A386;">16</span>/100</span>-->
              </p>
            </div>
            <div class="card-right">
              <span class="btu"
                :style="{ color: item.ipAddress ? '#26A386' : '#ACACAC', borderColor: item.ipAddress ? '#26A386' : '#BFBFBF' }">{{
                  item.assStatus === '1' ? '已交卷' : '未交卷' }}</span>
            </div>
          </div>
        </div>
        <div v-if="!listData.terminalList.length" class="no-data" :style="{ height: controlHight - 130 + 'px' }">
          <div>
      ...
      <div slot="drawer">
        <transition name="center-show" mode="in-out">
          <Statistical v-if="statisticalModel" ref="statisticalModel" :detailsId="this.detailsId"
            @closeDrawer="closeDrawer" @close="closeComponent" @error="error()">
          </Statistical>
        </transition>
      </div>
      <div slot="drawer">
      ...
      <Modal v-model="loginModal" :footer-hide="true" :mask-closable="false" width="50%" title="未登录人员详情表">
        <Nologin v-if="loginModal" ref="loginModal" :detailsId="this.detailsId" @closeDrawer="closeDrawer"
          @close="closeComponent" @error="error()">
        </Nologin>
      </Modal>
    </div>
  </div>
...
import {
  initAccessControlDetailsById,
  operatePointInfo,
  initAssessmentIsStatical
    ...
    initFormData() {
      ...
      initAccessControlDetailsById(params).then(res => {
        if (res.data.status) {
          ...
          this.examinationName = data.ass.examinationName
          this.listData = data
          setTimeout(() => {
            this.initFormData()
          }, 1000 * 60)
