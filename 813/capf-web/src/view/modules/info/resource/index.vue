<!--
 * @description datamanagement- ${comments}--index
 * @author yxm
 * @date 2022-04-07
 * -->
<template lang="html">
    ...
    <div slot="content">
      ...
      >
        <div slot="search" class="search-info">
          ...
          <div class="search-box">
            <span class="search-tip">资料标题：</span>
            <Input
              type="text"
              v-model="searchParams.title"
              placeholder="输入资料标题查询"
              :clearable="false"
              :maxlength="20"
              ref="title"
            />
...
<script>
import {
  initDataManagementList,
  deleteDataManagementByIds
...
export default {
  ...
  data() {
    return {
      canReset: this.isAuth('info:resource:query'), // 重置
      canSearch: this.isAuth('info:resource:query'), // 查询
      canSave: this.isAuth('info:resource:add'), // 添加
      canEdit: this.isAuth('info:resource:edit'), // 编辑
      canDelete: this.isAuth('info:resource:delete'), // 删除
      canDownload: this.isAuth('info:resource:download'), // 下载
  ...
  methods: {
    ...
    initListData() {
      ...
      // let name = this.searchParams.subjectName
      // if (!reg.test(name) && (name || name === 0)) {
      //   this.$Message.error('课目名称项请输入汉字或字母')
      //   return
      // }
      //  let title = this.searchParams.title
      //  if (!reg.test(title) && (title || title === 0)) {
      //   this.$Message.error('资料标题项请输入汉字或字母')
      //   return
      // }
      ...
      if (!validation.isPass) {
        ...
        this.loading = true
        initDataManagementList(this.searchParams)
          .then(res => {
            if (res.data.status) {
    ...
    async handleDelete() {
      ...
      deleteDataManagementByIds(params)
        .then(res => {
          if (res.data.status) {
