<!--
 * @description planmonth- ${comments}--新增/编辑查看/界面
 * @author yxm
 * @date 2022-04-07
 * -->
<template>
...
import {
  operatePlanmonthInfo,
  initPlanmonthDetailsById
  ...
  methods: {
    ...
    initFormData() {
      ...
      this.$nextTick(() => {
        ...
        if (this.isOperate) {
          initPlanmonthDetailsById(params)
            .then(res => {
              if (res.data.status) {
                const data = res.data.planMonthDO
                if (data == null) {
                  this.$Message.error('未查询到相关信息')
                  this.handleCancleModel()
                  return
                }
                // this.formData = res.data.planMonthDO
                ...
                this.formData.deptName = res.data.planMonthDO.deptName
    ...
    handleSubmit() {
      let params = Object.assign(
        { ...this.formData },
        {
          ...
          timeLimit: this.$moment(this.formData.timeLimit).format('YYYY-MM'),
