/**
 * planweek - 计划-周计划表 - 列表 columns 配置
 * @param _this
 * @returns {*}
 * @author yxm
 * @date 2022-04-07
...
module.exports = function(_this) {
  return {
    ...
    status: {
      ...
      align: 'center',
      render: (h, params) => {
        let value = params.row.status === '1' ? '待发布' : params.row.status === '2' ? '待审核' : params.row.status === '3' ? '已发布' : params.row.status === '4' ? '已完成' : '--'
        if (typeof (value) === 'undefined' || value === null || value === '') {
          value = '--'
        }
        return h('div', [
    ...
    personType: {
      ...
      align: 'center',
      render: (h, params) => {
        let value = params.row.personType === '1' ? '类型1' : params.row.personType === '2' ? '类型2' : '类型2'
        if (typeof (value) === 'undefined' || value === null || value === '') {
          value = '--'
        }
        return h('div', [
    ...
    TrainingContent: {
      ...
      render: (h, params) => {
        ...
        if (typeof (totalTimeH) === 'undefined' || totalTimeH === null || totalTimeH === '') {
          totalTimeH = '--'
        }
    ...
    examinationName: {
      ...
      render: (h, params) => {
        ...
        if (typeof (value) === 'undefined' || value === null || value === '' || params.row.assessment !== '1') {
          value = '--'
        }
        return h('div', [
          h('span', {
    ...
    option: {
      ...
      render: (h, params) => {
        ...
        let renderData = []
        if (_this.canEdit && params.row.status === '1' && params.row.createBy == listSub.userId) {
          renderData.push(
            h(
              'span', // 编辑
              {
        ...
        if (params.row.status === '1' && params.row.createBy == listSub.userId) {
          renderData.push(
            h(
              'span', // 删除
              {
        ...
        if(_this.canPublish && params.row.status === '1' && params.row.createBy == listSub.userId) {
          renderData.push(
            h('span', { // 发布
              class: 'c-success',
              props: {
        ...
        // if(_this.canBack && params.row.status === '3' && params.row.createBy == listSub.userId) {
        //   renderData.push(
        //     h('span', { // 撤销
        //       class: 'c-error',
        //       props: {
