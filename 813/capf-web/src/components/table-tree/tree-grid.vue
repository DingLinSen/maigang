 ...
 * @events  @on-row-click 单击行或者单击操作按钮方法
 ...
 -->
<template>
  <div :style="{ width: tableWidth, height: defaultHeight + 'px' }" class='autoTbale'>
    <div class="">
      <table class="table table-bordered autoTable" >
        <thead>
          <tr>
            <th v-for="(column,index) in cloneColumns" :style="tdStyle(column)" :key="index">
      ...
      <div class="tree-grid-table-wrapper">
        <table class="table table-bordered" id='hl-tree-table'>
          <tbody>
            <tr v-for="(item, index) in initItems" :key="item.id"
                v-show="show(item, index)" :class="{ 'child-tr':item.parent, 'selected-tr': selectedIndex === index }">
              <td v-for="(column,snum) in columns"
                :key="column.key" :style="tdStyle(column)">
                <label v-if="column.type === 'selection'" >
                  <input type="checkbox" :value="item.id" @click.stop="chooseRows(index, item.id)" v-model="checkGroup">
                </label>
                <div v-if="column.type === 'action'">
                  {{column.className}}
                  <span :type="action.type"
                    :class="column.className"
                    :style="{display: action.display}"
                ...
                <div v-if="!column.type" @click="toggle(index,item)"
                  :style="{ 'max-width': column.width + 'px' }" class="td-content">
                  ...
                  <span :content="renderBody(item,column)">
                    {{ renderBody(item,column) }}
                  </span>
                  <!-- <Tooltip :content="renderBody(item,column)">
                    {{ renderBody(item,column) }}
                  </Tooltip> -->
                </div>
              </td>
        ...
        <p style="text-align: center;font-size:16px;padding-top:200px" v-if="initItems.length < 1">暂无数据信息</p>
    </div>
  </div>
</template>
<script>
export default {
  ...
  methods: {
    resetTableHeight () { // 重置窗口可视高度测试
      this.defaultHeight = this.documentClientHeight - 100 - 55 - 57
    },
    chooseRows(index, id) {
      this.checkGroup = []
      this.checkGroup.push(id)
